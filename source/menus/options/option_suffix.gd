...
func _ready() -> void:
	...
	for i in new_options.size():
		...
		match typeof(new_options[i].opt_type):
			TYPE_INT, TYPE_FLOAT:
				update_text(str("<", new_options[i].opt_type, ">"), -80)
				
			TYPE_ARRAY:
				update_text(str("<", new_options[i].opt_type[GlobalOptions.current_array_opt], ">"), -80)
			...
			TYPE_BOOL:
				...
				check_sprite.sprite_frames = load("res://assets/images/checkboxThingie.res");
